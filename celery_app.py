# --- REMOVED MONKEY PATCH FROM HERE ---
...
log.info(f"Configuring Celery with Redis broker/backend: {redis_url}")
...
try:
    ...
    log.info("Redis connection successful for Celery setup check.")
    # Note: The worker will create its own instance usually
except Exception as e:
    log.error(f"Failed initial Redis connection check at {redis_url}: {e}", exc_info=True)
    # The application might still start, but tasks won't work

if __name__ == '__main__':
    # This allows running the worker directly, e.g., for debugging:
    # python -m celery -A celery_app:celery worker --loglevel=info -P eventlet
    celery.start()